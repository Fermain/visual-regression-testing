<script lang="ts">
	...
	let pathsText = $state(data.project.paths.join('\n'));
	let newPath = $state('');
...
<div class="flex-1 overflow-auto p-6">
	<form method="POST" action="?/update" use:enhance class="space-y-6">
		...
		<div class="flex justify-end gap-3">
			...
			<Button type="submit" disabled={!name.trim() || pathsList.length === 0}>Save Changes</Button>
		</div>
	</form>
	...
	<Card class="border-destructive/30">
		...
		<CardContent>
			<form method="POST" action="?/delete" use:enhance bind:this={deleteFormEl}>
				<Button variant="destructive" type="button" onclick={() => (deleteDialogOpen = true)}>
